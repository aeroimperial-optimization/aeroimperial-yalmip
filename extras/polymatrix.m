...
%
% P = POLYMATRIX(x,DEG,SIZE) with SIZE=[M N] construct an M-by-N matrix P of
...
%
% P = POLYMATRIX(x,DEG,SIZE,'symm') with SIZE = [N N], or its short-hand
...
%       SIZE(1)~=SIZE(2).
%
% [P,C,V] = POLYMATRIX(...) also returns the coefficient of the polynomial
...
P(SIZE(1),SIZE(2)) = x(1);  % dummy, will be overwritten
for i = 1:SIZE(1)
